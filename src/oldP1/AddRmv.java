...
public class AddRmv {
	...
	public static void addEntity(HashMap<String, String> entityData) {
		try {
			...
			Transformer transformer = transformerFactory
					.newTransformer(new StreamSource(OldConstants.XSL_ADD));

			for (Entry<String, String> param : entityData.entrySet()) {
				transformer.setParameter(param.getKey(), param.getValue());
			}

			transformer.transform(new StreamSource(new File(OldConstants.XML_DB)),
					new StreamResult(new File(OldConstants.XML_DB)));
	...
	public static void deleteEntity(String entityId) {
		try {
			...
			Transformer transformer = transformerFactory
					.newTransformer(new StreamSource(OldConstants.XSL_RMV));
